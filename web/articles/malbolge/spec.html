...
<title>Malbolge language specification</title>
...
<div class="host-note">
<p>July 10th, 2011: the original site has been gone since 2003, and this mirror
is a duplicate of sorts of
<a href="http://web.archive.org/web/20031202211441/http://www.mines.edu/students/b/bolmstea/malbolge/">the most recent archive.org mirror</a>.
This content is also <a href="http://www.lscheffer.com/malbolge.shtml">mirrored
by Lou Scheffer</a>, who has some interesting stuff on the langauge too.</p>
<p>I&rsquo;ve added additional links to the text.</p>

<p>Ben&rsquo;s original interpreter is available for download here too as a
<a href="malbolge.zip" title="Malbolge interpreter">ZIP file</a> and as a
<a href="malbolge.tar.gz" title="Malbolge interpreter">tarball</a>.</p>

<p>If you&rsquo;re interested in Malbolge, you&rsquo;d probably be interested
in <a href="dis.html">Dis</a> too.</p>
</div>

<div class="aside">
<p>I hereby relenquish any and all copyright on this language,
documentation, and interpreter; Malbolge is officially public domain.</p>
</div>

<h1>Malbolge &rsquo;98</h1>
...
<p>INTERCAL&rsquo;s constructs are certainly tortuous, but they are all too
flexible; you can, for instance, quite easily assign any number to a
variable with a single statement.</p>

<p>BrainF*** is lacking the flexibility which is INTERCAL&rsquo;s major
weakness, but it fails in that its constructs are far, far too intuitive.
Certainly, there are only 8 instructions, none of which take any
arguments&mdash;but it is quite easy to determine how to use those
instructions. Subtract 8 from the current number? With a simple
<code>--------</code> you are done! This kind of simple answer was unacceptable
to the author.</p>

<p>Hence the author created Malbolge. It borrows from machine, BrainF***, and
<a href="http://www.muppetlabs.com/~breadbox/intercal-man/s06.html">tri-INTERCAL</a>,
but put together in a unique way. It was designed to be difficult to use, and
so it is. It is designed to be incomprehensible, and so it is.</p>

<p>So far, no Malbolge programs have been written. Thus, we cannot give an
example.</p>

<p><strong>Malbolge</strong> is the name of Dante&rsquo;s Eighth Circle of
Hell, in which practitioners of deception (seducers, flatterers, simonists,
thieves, hypocrites, and so on) spend eternity.</p>
...
and data share the same space (the machine&rsquo;s memory segment), and there
are three registers. Machine words are ten trits (trinary digits) wide,
giving a maximum possible value of <em>59048</em> (all numbers are unsigned).
Memory space is exactly <em>59049 words long</em>.</p>

<p>The three registers are <var>A</var>, <var>C</var>, and <var>D</var>.
<var>A</var> is the accumulator, used for data manipulation. <var>A</var> is
implicitly set to the value written by all write operations on memory.
(Standard I/O, a distinctly non-chip-level feature, is done directly with the
<var>A</var> register.)</p>
...
<p><var>D</var> is the data pointer. It, too, is automatically incremented
after each instruction, but the location it points to is used for the data
...
<p>When the interpreter tries to execute a program, it first checks to see if
the current instruction is a graphical ASCII character (33 through 126). If it
is, it subtracts 33 from it, adds C to it, mods it by 94, then uses the result
as an index into the following table of 94 characters:</p>

<pre>+b(29e*j1VMEKLyC})8&amp;m#~W&gt;qxdRp0wkrUo[D7,XTcA"lI
v%{gJh4G\-=O@5`_3i&lt;?Z';FNQuY]szf$!BS/|t:Pn6^Ha</pre>
...
<p>When the interpreter parses the input file, it checks each non-whitespace
character with the process above. If any result is not one of the eight
characters below, the file will be rejected.</p>

<p>After the instruction is executed, 33 is subtracted from the instruction at
<var>C</var>, and the result is used as an index in the table below. The new
character is then placed at <var>C</var>, and then <var>C</var> is
incremented.</p>

<pre>5z]&amp;gqtyfr$(we4{WP)H-Zn,[%\3dL+Q;&gt;U!pJS72FhOA1C
B6v^=I_0/8|jsb9m&lt;.TVac`uY*MK'X~xDl}REokN:#?G"i@</pre>
...
<dt><kbd>*</kbd></dt>
<dd>rotates the trinary value of the cell pointed to by D to the right 1. The least significant trit becomes the most significant trit, and all others move one position to the left.</dd>
<dt><kbd>p</kbd></dt>
<dd>performs a tritwise &lsquo;op&lsquo; on the value pointed to by D with the contents of A. The op (don&rsquo;t look for pattern, it&rsquo;s not there) is:
...
<dt><kbd>/</kbd></dt>
<dd>converts the value in A to ASCII and writes it to stdout. Writing 10 is a newline.</dd>

<dt><kbd>v</kbd></dt>
<dd>indicates a full stop for the machine.</dd>  

<dt><kbd>o</kbd></dt>
<dd>does nothing, except increment <var>C</var> and <var>D</var>, as all other instructions do.</dd>
</dl>


<h2>Turing-Completeness</h2>

<p>Though I have not proven it, I <em>think</em> Malbolge to be Turing-complete.
To be Turing-complete, there must be some data construct which can be
used to do any mathematical calculation. I believe that using <kbd>*p</kbd> in
various clever ways on the tritwords can fulfill this requirement.</p>

<p>Turing-completeness also requires three code constructs: <em>sequential
execution</em> (which Malbolge obviously has), <em>repetition</em> (provided
by the <kbd>i</kbd> and, indirectly, <kbd>j</kbd> instructions), and
<em>conditional-execution</em> (provided, I believe, by self-modifying code
and altering <kbd>i</kbd> destinations).</p>

<p>I do have my doubts, particularly about data constructs, but I
<em>think</em> this works...</p>

<h2>Appendix: Trinary Conversion Table</h2>

<p>Trinary to ASCII to decimal to hex table, provided, strangely enough,
for the convenience of Malbolge programmers.</p>
